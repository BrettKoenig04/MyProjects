...
def request_dishes(dish_input):
    ...
    # Set max attemps to contact API with requests, as long as an error hasn't occured before.
    for i in range(3):
        try:
...
def check_cache(dish_input):
    ...
    user_input = input("Would you like to clear cached items for meals? Yes/No: ")
    while True:
...
def convert_response(response):
    ...
    for items in response["hits"]:
        recipe_name.append(items["recipe"]["label"].title())
...
def search_restaurant(dishes_found):
    print("\n")
    yelp_dish = input("Please enter name of dish you'd like to search on yelp: ").title().strip()
    
    

    while yelp_dish not in dishes_found.keys():
        yelp_dish = input("You typed in an incorrect dish name, please try again: ").title().strip()

    print("Searching for resteraunts that may contain this dish.\n")
    return yelp_dish